    ...
    async createPayment(
        ...
        if (dto.type === 'credits') {
            if (!dto.credits || dto.credits < 10 || dto.credits > 5000) {
                throw new BadRequestException('Số credits phải từ 10 đến 5000');
            }
            ...
            amount = getSubscriptionPrice(dto.subscriptionTier, billingCycle);
            const tierName =
        ...
        const qrResult = await this.sePayService.createQR(amount, paymentId);
        ...
        return {
            ...
            qrUrl: qrResult.QR,
            bankInfo: {
                bankName: qrResult.bank?.bank_name || 'MB Bank',
                accountNumber: qrResult.bank?.account_number || '',
                accountName: qrResult.bank?.account_name || 'EXAMIO',
            },
        };
    }
