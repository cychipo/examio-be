...
export class RedisService implements OnModuleInit, OnModuleDestroy {
    ...
    async delPattern(pattern: string): Promise<void> {
        ...
        console.log(`[Redis] Deleting ${keys.length} keys matching pattern: ${pattern}`);

        if (keys.length > 0) {
            // Xóa theo batch để tránh command quá lớn
            const batchSize = 100;
